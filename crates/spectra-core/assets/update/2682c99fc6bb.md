...
1. **Determine the requirement source**
   ...
   - If conversation context mentions `.cursor/plans/<name>.md` and the file exists, ask whether to use the plan file or conversation context.
...
4. **Create the change directory**
   ...
   ```bash
   spectra new change "<name>" --schema no-spec --agent cursor
   ```
