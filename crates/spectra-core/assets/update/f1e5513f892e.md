...
This tool can discover bare plan names under `.cursor/plans/` and also accepts explicit existing plan paths.
...
1. **Resolve the requirement source and target hint**
   ...
   c. **Plan-directory candidate** — Otherwise, if the bare argument resolves to a plan under `.cursor/plans/`, set `requirement_source` to that plan and select `target_change` separately.
