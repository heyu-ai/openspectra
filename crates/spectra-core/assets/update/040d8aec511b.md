...
6. **User confirmation**
   ...
   - Fetch the archive sub-flow with `spectra instructions --skill commit-archive --agent github-copilot` and execute it as step 6a before continuing to step 7. Do not perform any archive action from memory; the fetched sub-flow is the only source for step 6a.
