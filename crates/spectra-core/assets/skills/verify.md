...
5. **Verify Completeness**
   ...
   **Spec Coverage**:
   With delta specs in `{{SPEC_DIR}}changes/<name>/specs/`, fetch `spectra instructions --skill verify-spec-coverage --agent {{TOOL}}` once and run its Spec Coverage checks. Without delta specs, skip the fetch.
