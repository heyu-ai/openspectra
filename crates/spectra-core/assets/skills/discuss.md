...
Read `{{SPEC_DIR}}LANGUAGE.md` before anything else in this skill.
...
| --- | --- |
| New requirement discovered | `{{SPEC_DIR}}specs/<capability>/spec.md` |
| Design decision made | `{{SPEC_DIR}}changes/<name>/design.md` |
| Scope changed | `{{SPEC_DIR}}changes/<name>/proposal.md` |
| New work identified | `{{SPEC_DIR}}changes/<name>/tasks.md` |
| Vocabulary drift | `{{SPEC_DIR}}LANGUAGE.md` |

**Vocabulary drift** means a recurring concept is missing, ambiguous, or pulling away from Step 0 vocabulary. Name it in the conclusion and direct capture to `{{SPEC_DIR}}LANGUAGE.md`. The conclusion summary SHALL preserve this contract — do not silently rewrite the term in artifacts without recording the drift.
