...
1. **Determine the requirement source**
   ...
   - If conversation context mentions `{{PLAN_DIR}}<name>.md` and the file exists, ask whether to use the plan file or conversation context.
...
3. **Scan existing specs for relevance**
   ...
   Skip this for `no-spec`. Otherwise list `{{SPEC_DIR}}specs/*/spec.md`, compare spec IDs to the requirement, read short Purpose snippets for up to 3 candidates, and show related specs only as info.
...
4. **Create the change directory**
   ...
   ```bash
   spectra new change "<name>" --agent {{TOOL}}
   ```

   No capability-level behavior:

   ```bash
   spectra new change "<name>" --schema no-spec --agent {{TOOL}}
   ```
